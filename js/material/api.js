...
/**
 * Update field-field tertentu di satu baris master_data.
 * @param {string|number} id        - ID baris (UUID atau bigint)
 * @param {object}        fields    - Kolom-kolom yang ingin diperbarui
 * @returns {Promise<{status:'ok'}>}
 */
export async function apiUpdateMasterData(id, fields) {
    const { error } = await supabase
        .from('material_master_data')
        .update(fields)
        .eq('id', id);
...
/**
 * Hapus satu baris master_data (hanya boleh jika status 'pending').
 * RLS di database akan menolak jika user bukan admin.
 * @param {string|number} id - ID baris
 * @returns {Promise<{status:'ok'}>}
 */
export async function apiDeleteMasterData(id) {
    const { error } = await supabase
        ...
        .delete()
        .eq('id', id)
        .eq('status', 'pending');  // safety guard di sisi client
...
/**
 * Ambil riwayat klaim material.
 * @param {object} opts
 * @param {string}  opts.poNumber   - Filter by PO (opsional)
 * @param {number}  opts.limit      - Batas rows (default 200)
 * @param {number}  opts.page       - Halaman (default 1)
 * @returns {Promise<{data: object[], total: number}>}
 */
export async function apiGetClaims({ poNumber = '', limit = 200, page = 1 } = {}) {
    ...
    if (poNumber) q = q.eq('po_number', poNumber);
