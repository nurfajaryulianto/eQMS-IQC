...
function renderInspectionLog(data) {
    ...
    tbody.innerHTML = data.map((d, i) => {
        ...
        return `<tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
            ...
            <td style="${TD}" title="${esc(matFullTitle)}">
                <div style="color:#34d399;font-weight:700;font-size:11px;overflow:hidden;text-overflow:ellipsis;">${esc(matCode)}</div>
                ${matDesc && matDesc !== matCode ? `<div style="color:rgba(255,255,255,0.5);font-size:10px;overflow:hidden;text-overflow:ellipsis;">${esc(matDesc)}</div>` : ''}
            </td>
